...
class UserTest extends FunSuite {
  ...
  override  def beforeEach(context: BeforeEach): Unit = {
    user1 = new User(name, gems, deck1, hand, board1)
    user2 = new User(name, gems, deck2, hand, board2)
  }
  ...
  test("When using a setter, the cards should change and be the same for both boards") {
  ...
  test("A computer and a card can be created with all the necessary data") {
    assertEquals(new User(name, gems, deck2, hand, board1), user2)
  ...
  test("The hash code of a User is consistent with equals") {
    assertEquals(new User(name, gems, deck1, hand, board1).##, user1.##)
  }
